...
! defined( 'WOOSCP_VERSION' ) && define( 'WOOSCP_VERSION', '2.8.2' );
				...
				function wooscp_load_compare_bar() {
					...
					if ( ! empty( $wooscp_products ) ) {
						foreach ( $wooscp_products as $wooscp_product ) {
							...
							$wooscp_output .= '<span class="wooscp-bar-item-img hint--top" aria-label="' . esc_attr( wp_strip_all_tags( $wooscp_product_obj->get_formatted_name() ) ) . '">' . $wooscp_product_obj->get_image( 'wooscp-small' ) . '</span>';
				...
				function wooscp_get_compare_table( $ajax = true ) {
					...
					if ( is_array( $wooscp_products ) && ( count( $wooscp_products ) > 0 ) ) {
						...
						foreach ( $wooscp_products as $wooscp_product ) {
							...
							$wooscp_products_data[ $wooscp_product ]['title'] = '<a href="' . $product->get_permalink() . '" draggable="false">' . wp_strip_all_tags( $product->get_formatted_name() ) . '</a>';
							...
							if ( in_array( 'additional', $saved_fields, true ) ) {
								...
								ob_end_clean();
								$wooscp_products_data[ $wooscp_product ]['additional'] = $additional;
							}
                        ...
                        <div class="wooscp-inner">
                            <div class="wooscp-table">
                                <div class="wooscp-table-inner">
									<?php echo( get_option( '_wooscp_close_button', 'yes' ) === 'yes' ? '<span id="wooscp-table-close" class="wooscp-table-close"><span class="wooscp-table-close-icon hint--left" aria-label="' . esc_html__( 'Close', 'wooscp' ) . '"></span></span>' : '' ); ?>
                            ...
                            <div class="wooscp-bar">
								<?php if ( get_option( '_wooscp_bar_settings', 'yes' ) === 'yes' ) { ?>
                                    <div class="wooscp-bar-settings hint--top"
                                         aria-label="<?php esc_html_e( 'Select fields', 'wooscp' ); ?>"></div>
								<?php } ?>
								<?php if ( get_option( '_wooscp_bar_add', 'yes' ) === 'yes' ) { ?>
